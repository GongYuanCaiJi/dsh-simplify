...
async function addChangedLines(
  ...
  return Promise.all(files.map(async (file) => {
    ...
    const result = await exec(ctx, "git", diffArgs(options, ref, file.path), { cwd });
...
export async function getChangedFiles(
  ...
  if (options.files.length > 0) {
    ...
    return addChangedLines(ctx, cwd, options, files, options.ref);
  }
  ...
  const result = await exec(ctx, "git", args, { cwd });
  if (result.code === 0) {
    const files = parseDiffOutput(result.stdout);
    if (files.length > 0) return addChangedLines(ctx, cwd, options, files, options.ref);
  }

  // Fallback: diff against previous commit
  const fallback = await exec(ctx, "git", ["diff", "--name-status", "HEAD~1"], { cwd });
  if (fallback.code === 0) {
    const files = parseDiffOutput(fallback.stdout);
    return addChangedLines(ctx, cwd, options, files, "HEAD~1");
  }
